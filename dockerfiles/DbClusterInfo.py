...
class dbClusterInfo():
    ...
    def __readClusterNodeInfo(self):
        ...
        # Get cm server info
        for dbNode in self.dbNodes:
            self.__readCmsConfig(dbNode, nodeNames)
    ...
    def __readCmsConfig(self, masterNode, nodeNames):
        """
        function : Read cm server config on node.
        input : []
        output : NA
        """
        self.__readCmsConfigForMutilAZ(masterNode, nodeNames)

    def __readCmsConfigForMutilAZ(self, masterNode, nodeNames):
        """
        """
        cmsListenIps = None
        cmsHaIps = None
        ...
        for i in range(masterNode.cmsNum):
            ...
            if masterNode.name in hostNames:
            ...
            for j, name in enumerate(nodeNames):
                if name == masterNode.name:
                    continue
                if name not in hostNames:
                    continue
                dbNode = self.getDbNodeByName(name)
                if dbNode is None:
                    raise Exception(ErrorCode.GAUSS_502["GAUSS_50204"]
                                    % ("CMServer configuration on host [%s]"
                                       % masterNode.name)
